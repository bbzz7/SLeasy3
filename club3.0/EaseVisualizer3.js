// Copyright 2020 GreenSock Inc. See http://greensock.com/gsap/
;(function ($) {
    var htmlLink = "EaseVisualizer3.html";
    // var htmlLink = "js/EaseVisualizer/EaseVisualizer3.html";
    var id = 0;
    var timeline;
    var $menuEases;
    var highlightTween;
    ...
    var _createElement = function(type, ns) {
        var e = document.createElementNS ? document.createElementNS((ns || "http://www.w3.org/1999/xhtml").replace(/^https/, "http"), type) : document.createElement(type); //some servers swap in https for http in the namespace which can break things, making "style" inaccessible.
        return e.style ? e : document.createElement(type); //some environments won't allow access to the element's style when created with a namespace in which case we default to the standard createElement() to work around the issue. Also note that when GSAP is embedded directly inside an SVG file, createElement() won't allow access to the style object in Firefox (see https://greensock.com/forums/topic/20215-problem-using-tweenmax-in-standalone-self-containing-svg-file-err-cannot-set-property-csstext-of-undefined/).
    };
    ...
    function loaded(html, vis) {
        ...
        $(".command").on("copy", function(e) {
            _copyElement.value = document.getSelection().toString(); //_copyElement.value = '"' + data.currentEase + '"';
    ...
    function parseEaseClass(name, short) {
        name = name.split(".")[0];
        if (short) {
            return (name === "rough") ? "rough" : (name === "steps") ? "steps" : name;
        }
        return (name === "rough") ? "rough" : (name === "steps") ? "steps" : (name === "Custom") ? "CustomEase" : name;
    }
    ...
    function highlightMenuEase(name) {
        ...
        $menuEases.siblings(".ease_type_section").css("visibility", (name === "rough" || name === "steps" || name === "slow" || name === "none" || name === "Custom") ? "hidden" : "visible");
    }
    ...
    function checkVertical(vis) { //elastic.out needs more room up top.
        ...
        var tall = (target_select === "graph" && (main_ease_class_select === "CustomEase" || (main_ease_class_select === "elastic" || main_ease_class_select === "back" || (main_ease_class_select === "rough" && vis.find(".rough_ease_class_select").val() === "elastic" && vis.find(".rough_ease_type_select").val() === "out"))));
        if (tall) {
    ...
    function createGraph(vis) {
        ...
        if (customString) {
            ...
            simplified = ease.custom || ease.rawBezier ? path : PathEditor.simplifySVG(path, {tolerance: (precision === 1) ? 3 : 1, cornerThreshold: (mainEase === "bounce") ? 130 : (mainEase === "steps" || mainEase === "rough") ? 180 : 0});
...
//jQuery(function () {
var dimmer, activeOverlay, overlayZIndex, exportedRoot, overlayOnComplete;
